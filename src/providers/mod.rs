...
fn create_provider_with_url_and_options(
    ...
    match name {
        ...
        // If the name looks like "org/model" (e.g. "google/gemini-3-flash-preview"),
        // route it through OpenRouter which supports arbitrary model IDs.
        name if name.contains('/') && !name.starts_with("custom:") && !name.starts_with("anthropic-custom:") && !name.starts_with("http") => {
            tracing::info!(model = name, "Routing slash-format model ID through OpenRouter");
            // Resolve credential using 'openrouter' provider name so OPENROUTER_API_KEY is found
            let or_key = resolve_provider_credential("openrouter", None);
            let or_key_ref = or_key.as_deref();
            Ok(Box::new(openrouter::OpenRouterProvider::new(or_key_ref)))
        }
        _ => anyhow::bail!(
